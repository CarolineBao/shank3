...
preprocessing <- function (gene, animal, genome_nm, path, len_upstream, len_downstream, track) {
  ...
  gene_features <- tryCatch({read.table(paste(dir_path, "input_data/",paste("annotated", genome_nm, track, sep="_"), ".txt", sep=''), header = F, stringsAsFactors = F)
                                }, error = function(e) {
                                  stop(paste("Expected ", paste(dir_path, "input_data/",paste("annotated", genome_nm, track, sep="_"), ".txt", sep=''), " but did not find it. Is the file named correctly?", sep=""))
                                })
