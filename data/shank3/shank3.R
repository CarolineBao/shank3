...
shank_gene <- read.table(paste(FILE_PATH, "input_data/","slc32a1", "_gene.txt", sep=''), header = T, stringsAsFactors = F)
shank_all <- read.table(paste(FILE_PATH, "input_data/","slc32a1", "_features.txt", sep=''), header = T, stringsAsFactors = F) #features, aka exons, introns, utr, promoter
...
shank.match.motif.pos <- matchMotifs(jaspar_motifs, shank_gene, out = "positions", genome = GENOME)
...
write.table(shank.match.motif.df, paste(FILE_PATH, "results/test_", "slc32a1", "_region_TF_motifs_hs_ms.txt", sep=''), sep = "\t", quote = F, row.names = F)
shank.match.motif.df.onlystart<-shank.match.motif.df
shank.match.motif.df.onlystart$end<-shank.match.motif.df$start
...
intersection_by_bp_window <- function(gene, motifs, window_size, shift=0, tfbs_name="") {
    ...
    write.table(lapply(as.data.frame(ans, stringsAsFactors=F), as.character), 
                paste(FILE_PATH, "results/interneuron_enhancers/gene_intersect_frequencies_", window_size, "_bps",tfbs_name,".txt", sep=''), sep="\t", col.names=FALSE, quote = F, row.names = F)
    ans
}
...
print(shank.match.motif.df.onlystart)
for (val in c('SLC32A1', 'NPY', 'VIP')){
    results<-intersection_by_bp_window(shank_gene, shank.match.motif.df.onlystart, 100, tfbs_name=val)
}
